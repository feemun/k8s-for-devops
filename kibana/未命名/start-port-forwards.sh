...
# 示例：kubectl port-forward --address 0.0.0.0 deploy/kibana 15601:5601 -n kibana

# 方案三：转发到 Pod（指定具体 Pod）
# 命令：kubectl port-forward --address 0.0.0.0 pod/<pod-name> <本机端口>:<容器端口> -n <namespace>
# 示例：kubectl port-forward --address 0.0.0.0 pod/kibana-xxxxxxxx 15601:5601 -n kibana
...
# 默认执行：转发到 Service 的 5601
kubectl port-forward --address 0.0.0.0 service/kibana-service 15601:5601 -n kibana &